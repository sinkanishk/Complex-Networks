        ...
        #----------------------------------------------------------------------
            ...
            for n1 in commu[c]:
                for n2 in commu[c]:
                    ...
                    try:
                        cicj+=(ci*cj)
                        #cicj += (ci*cj*1.0)/(normi*normj*1.0)
                    except:
                        cicj +=0
            cicj = cicj/2

            try:
                norm = 1.0/(2.0*sum([E[l] for l in layer]) + E12*1.0) #norm = 1/(2*|E1| + 2*|E2| + |E12|)
                mod = norm*1.0*(Aij*1.0 - (norm*1.0*cicj))
