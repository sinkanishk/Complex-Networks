...
def apply_p1_parameter(nb_layers, layers, couplings_links, cross_communities, p1):
	...
	for i in range(0, nb_layers-1):
		...
		print("after applying p1, number of edges within commu: ",nb_link_inside_cross_coms)
...
def apply_p2_parameter(nb_layers, layers, couplings_links, intra_coms, cross_communities, p2):
	...
	for i in range(0, nb_layers-1):
		...
		# (b)	intra_coms is a dict whichc ontains all single layer communities in each layer
	...
	if p > p_temp:
		...
		while p > p_temp and nb_new_coupling < total_possible_nb_coupling:
			...
			if (node_l1, node_l2) not in couplings_links[i_layer][i_layer+1]:
				...
				nb_new_coupling += 1
				p_temp = (1.0 * nb_link_inside_cross_coms + nb_new_coupling) / (nb_link_inside_cross_coms + nb_link_outside_cross_coms+ nb_new_coupling)
		...
		p_act = (1.0 * nb_link_inside_cross_coms) / (nb_link_inside_cross_coms + nb_link_outside_cross_coms)
		print "actual p value : %f" % p_act
		sys.exit()
